...
def make_dataset(root, source):
    ...
    if not os.path.exists(source):
        print("Setting file %s for CVPR_LE dataset doesn't exist." % (source))
        sys.exit()
    else:
        clips = []
...
def ReadSegmentRGB(path, offsets, new_height, new_width, new_length, is_color, name_pattern, duration):
    ...
    sampled_list = []
    clip_name = os.path.basename(os.path.normpath(path))
...
class cvpr_le(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.modality == "rgb":
            clip_input, clip_name = ReadSegmentRGB(path,
                                        offsets,
                                        self.new_height,
                                        self.new_width,
                                        self.new_length,
