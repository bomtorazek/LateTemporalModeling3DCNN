...
import datasets
from two_stream_bert import option, data_prep_super, build, utils, learn_super, data_config, optimization
...
def main():
    ...
    input_size, width, height = data_prep_super.get_size(args)

    saveLocation = args.save_dir + "/" + args.dataset + "_" + args.arch + "_split" + str(args.split) + "_mixtype_" + str(args.mix_type) +"_super"
    if args.randaug:
    ...
    length, modality, is_color, scale_ratios, clip_mean, clip_std = data_prep_super.get_data_stat(args)

    train_transform, val_transform = data_prep_super.get_transforms(input_size, scale_ratios, clip_mean, clip_std, args)
    ...
    if args.evaluate:
        acc1,acc3,lossClassification = learn_super.validate(val_loader, model, criterion, modality, args, length, input_size)
        return

    for epoch in range(startEpoch, args.epochs):
        learn_super.train(train_loader, model, criterion, optimizer, epoch, modality, args, length, input_size, writer)
